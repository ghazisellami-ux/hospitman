...
import { riskItems, categoryLabels, probabilityLabels, impactLabels, type Language } from '@/lib/demoData';
...
export default function RiskPage() {
  ...
  const catBadge = (c: string) => {
    ...
    return { cls: colors[c] || 'badge-neutral', label: categoryLabels[c]?.[l] || c };
  };
  ...
  return (
    <div className="animate-in">
      ...
      <div className="kpi-grid" style={{ gridTemplateColumns: 'repeat(4, 1fr)', marginBottom: 24 }}>
        <div className="kpi-card danger"><div className="kpi-label">{l === 'fr' ? 'Critiques (≥15)' : 'Critical (≥15)'}</div><div className="kpi-value" style={{ fontSize: 28, color: '#ef4444' }}>{risks.filter(r => r.score >= 15).length}</div></div>
        <div className="kpi-card warning"><div className="kpi-label">{l === 'fr' ? 'Élevés (10-14)' : 'High (10-14)'}</div><div className="kpi-value" style={{ fontSize: 28, color: '#f59e0b' }}>{risks.filter(r => r.score >= 10 && r.score < 15).length}</div></div>
        <div className="kpi-card info"><div className="kpi-label">{l === 'fr' ? 'Moyens (5-9)' : 'Medium (5-9)'}</div><div className="kpi-value" style={{ fontSize: 28, color: '#3b82f6' }}>{risks.filter(r => r.score >= 5 && r.score < 10).length}</div></div>
        <div className="kpi-card success"><div className="kpi-label">{l === 'fr' ? 'Faibles (<5)' : 'Low (<5)'}</div><div className="kpi-value" style={{ fontSize: 28, color: '#10b981' }}>{risks.filter(r => r.score < 5).length}</div></div>
      </div>
      ...
      <div className="data-table-wrapper">
        <table className="data-table">
          <thead><tr><th>#</th><th>{t('common.description', lang)}</th><th>{l === 'fr' ? 'Catégorie' : 'Category'}</th><th>{l === 'fr' ? 'Probabilité' : 'Prob.'}</th><th>Impact</th><th>Score</th><th>{l === 'fr' ? 'Responsable' : 'Owner'}</th><th>{t('common.status', lang)}</th></tr></thead>
          <tbody>
            {risks.map((r) => {
              const cat = catBadge(r.category);
              return (
                <tr key={r.id}>
                  ...
                  <td style={{ fontSize: 13 }}>{probabilityLabels[r.probability]?.[l] || r.probability}</td>
                  <td style={{ fontSize: 13 }}>{impactLabels[r.impact]?.[l] || r.impact}</td>
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3 className="modal-title">{l === 'fr' ? 'Nouveau Risque' : 'New Risk'}</h3>
              <button className="modal-close" onClick={() => setShowModal(false)}><X size={20} /></button>
            </div>
            <div className="form-group">
              <label className="form-label">{t('common.description', lang)}</label>
              <textarea className="form-textarea" placeholder={l === 'fr' ? 'Décrivez le risque identifié...' : 'Describe the identified risk...'} value={form.desc} onChange={(e) => setForm({ ...form, desc: e.target.value })} />
            </div>
            <div className="form-row">
              <div className="form-group">
                <label className="form-label">{l === 'fr' ? 'Catégorie' : 'Category'}</label>
                <select className="form-select" value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}>
                  {categories.map(c => <option key={c} value={c}>{categoryLabels[c]?.[l] || c}</option>)}
                </select>
              </div>
              <div className="form-group">
                <label className="form-label">{l === 'fr' ? 'Probabilité' : 'Probability'}</label>
                <select className="form-select" value={form.probability} onChange={(e) => setForm({ ...form, probability: e.target.value })}>
                  {probabilities.map(p => <option key={p} value={p}>{probabilityLabels[p]?.[l] || p}</option>)}
                </select>
              </div>
            </div>
            <div className="form-row">
              <div className="form-group">
                ...
                <select className="form-select" value={form.impact} onChange={(e) => setForm({ ...form, impact: e.target.value })}>
                  {impacts.map(i => <option key={i} value={i}>{impactLabels[i]?.[l] || i}</option>)}
                </select>
              </div>
              <div className="form-group">
                <label className="form-label">{l === 'fr' ? 'Responsable' : 'Owner'}</label>
            ...
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: 12, marginBottom: 16 }}>
              <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{l === 'fr' ? 'Score calculé' : 'Calculated Score'}: </span>
            ...
            <div className="modal-actions">
              ...
              <button className="btn btn-primary" onClick={handleAdd}>{l === 'fr' ? 'Ajouter Risque' : 'Add Risk'}</button>
            </div>
          </div>
        </div>
      )}
